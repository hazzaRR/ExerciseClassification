...
def run_experiment(clf, sklearn_clf=False, uni_ts_clf=False):
    ...
    for dataset_type in os.listdir(DATA_PATH):
        ...
        for dataset in os.listdir(CURRENT_DATASET):
            ...
            if uni_ts_clf:
                print(col_ensemble_experiment(X, y, clf, f"results/{clf_name}/{dataset}_cv", dataset))

            else:
            # print(X_train)
                print(time_series_experiment(X, y, clf, f"results/{clf_name}/{dataset}_cv", dataset))
